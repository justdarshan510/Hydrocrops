...
interface DashboardProps {
  dataset: HydroponicImageRecord[];
  setDataset: React.Dispatch<React.SetStateAction<HydroponicImageRecord[]>>;
  filename: string;
  setFilename: React.Dispatch<React.SetStateAction<string>>;
}

export default function Dashboard({ dataset, setDataset, filename, setFilename }: DashboardProps) {
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        <div className="flex h-16 items-center px-6">
          <div 
            className="flex items-center gap-2 overflow-hidden whitespace-nowrap text-xl font-bold text-primary cursor-pointer"
            onClick={() => setLocation('/')}
          >
        ...
        <div className="p-4 space-y-2">
          <Button 
            variant="outline" 
            size="sm" 
            className="w-full text-xs font-semibold text-emerald-600 hover:text-emerald-700 border-emerald-100 hover:bg-emerald-50 flex items-center justify-center gap-1.5"
            onClick={() => setLocation('/scan')}
          >
            {isSidebarOpen ? '← Exit to Scanner' : '←'}
          </Button>
